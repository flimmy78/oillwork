...
typedef struct
{
	unsigned char DEV;//显示器选择0=键盘点阵屏；1=android平板显示屏
	int DEVKeyx;      //键盘点阵屏显示设备号
	int comFd;       //平板显示通讯串口
	int interfaceID; //前一次显示的界面ID号	
	unsigned char txBuffer[DSP_LEN_MAX]; //显示发送数据缓存
	int txLen; //显示发送数据长度
	unsigned char txMark; //显示发送标识
	unsigned char txFrame; //数据发送帧号
	
	//del SEM_ID semIdTx;
	pthread_mutex_t semIdTx; //数据发送操作信号量
	unsigned char rxBuffer[DSP_LEN_MAX]; //接收数据缓存
	
	int rxLen; //接收数据长度
	unsigned char rxMark; //接收完整标识
	int tIdDspRx; //显示接收任务ID
	int tIdDsp; //显示任务ID
	int Contrast; //显示对比度
}DspParamStructType;
...
int dspVideo(int nozzle)
{
	...
	*timer=0;
    ...
    pthread_mutex_unlock(&dsp->semIdTx); //fj:
	//semGive(dsp->semIdTx);
...
int dsp(int nozzle, int id, unsigned char *buffer, int nbytes)
{
	...
	//判断枪选
	if(DSP_NOZZLE_1==nozzle)			{dsp=&DspParamA;	timer=&dspTimerA;iptparam = &IptParamA;	}
	else if(DSP_NOZZLE_2==nozzle)		{dsp=&DspParamB;	timer=&dspTimerB;iptparam = &IptParamB;	}
	else								return 0;//yym 增加返回0，原始无返回值
	...
	if(DSP_DEV_KEYBOARD==dsp->DEV)
	{
		...
		case DSP_CARD_PRETREAT:	
			...
			if(id==dsp->interfaceID)	
				tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR_NO);
			else										
				tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR);
			break;
		...
		case DSP_CARD_BALANCE:							
			...
			else if(iptparam->EtcOilFlg==1)//szb_fj_20171120,add,ETC加油
			{
				memset(dsp_buffer,0,sizeof(dsp_buffer));
				memcpy(dsp_buffer,iptparam->EtcSelCardInf,16);
				for(i=0;i<16;i++)
				{
					if(dsp_buffer[i]==0)
						break;
				...
				if(iptparam->etc_limit_car==1)//szb_fj_20171120,add,限车号
				{
					memset(dsp_buffer+i,' ',sizeof(dsp_buffer)-i);
					memcpy(dsp_buffer+10,"限车号",strlen("限车号"));
					tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 6, 0*8, dsp_buffer,16);
				}
				else
					tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 6, 0*8, dsp_buffer,i);
		...
		case DSP_CARD_OILLING:							
			...
			if(iptparam->EtcOilFlg==1)//szb_fj_20171120,ETC加油
			{
				memset(dsp_buffer,0,sizeof(dsp_buffer));
				memcpy(dsp_buffer,iptparam->EtcSelCardInf,16);
				for(i=0;i<16;i++)
				...
				if(iptparam->etc_limit_car==1)//szb_fj_20171120,限车号
				{
					memset(dsp_buffer+i,' ',sizeof(dsp_buffer)-i);
					memcpy(dsp_buffer+10,"限车号",strlen("限车号"));
					tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 6, 0*8, dsp_buffer,16);
		...
		case DSP_INQ_SELECT:
        	...
        	else if(7==buffer[0])//szb_fj_20171120,add
			{
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 0, 0*8, "29.版本信息", 11);
			}
		...
		case DSP_SET_SELECT:
			...
			if(0==buffer[0]){
				...
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 0, 0*8, "29.ETC功能设置", 14);
			}
		...
		case DSP_BANK_OILLING:				//银行卡加油中界面
			...
			if(id==dsp->interfaceID)	
				...
				tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR);			
			break; 
        case DSP_ETC_FUN_COUR://szb_fj_20171120,ETC功能设置界面
			if(buffer[0]==ETC_FUN_NO)
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 0, 0*8, "当前:禁止ETC功能", 16);
			else if(buffer[0]==ETC_FUN_OK)
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 0, 0*8, "当前:允许ETC功能", 16);

			tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 4, 0*8, "1.禁止", strlen("1.禁止"));
			tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 6, 0*8, "2.允许", strlen("2.允许"));
			//szb_fj_20171120,首次显示的界面需要清屏，非首次显示界面不清屏
			if(id==dsp->interfaceID)	tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR_NO);
			else										tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR);
			break;
		case DSP_ETC_PIN_INPUT://szb_fj_20171120,ETC验证码输入
			tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 2, 0*8, "请输入验证码:", strlen("请输入验证码:"));
			if(nbytes==1 || nbytes==3)
			{
				if(nbytes==1)
				{
					memset(dsp_buffer,' ',sizeof(dsp_buffer));
					tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,6, 0*8, dsp_buffer, 16);
					memcpy(dsp_buffer+16-buffer[0],"****",buffer[0]);
					tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,4, 0*8, dsp_buffer, 16);
				}
				else
				{
					memset(dsp_buffer,0,sizeof(dsp_buffer));
					sprintf(dsp_buffer,"天线状态:%x",buffer[1]);
					tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,6, 0*8, dsp_buffer, strlen(dsp_buffer));
					memset(dsp_buffer,' ',sizeof(dsp_buffer));
					memcpy(dsp_buffer+16-buffer[0],"****",buffer[0]);
					tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,4, 0*8, dsp_buffer, 16);
				}
			...
			else if(nbytes==2)
			{
				...
				if((buffer[1]&0x01)==0x01)
					memcpy(dsp_buffer, "卡不存在",8);		//送显	
				else if((buffer[1]&0x20)==0x20)
					memcpy(dsp_buffer, "OBU被锁",7);		//送显	
				else if((buffer[1]&0x40)==0x40)
					memcpy(dsp_buffer, "OBU被拆动",9);		//送显	
				else if((buffer[1]&0x80)==0x80)
					memcpy(dsp_buffer,"OBU电量低",9);		//送显
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,6, 0*8, dsp_buffer, 16);
			}
			if(id==dsp->interfaceID)	tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR_NO);
			else										tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR);
			break;
		case DSP_CAR_NOYIZHI://szb_fj_20171120,ETC车牌号不一致
			tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 2, 0*8, "  车牌号不一致  ", strlen( "  车牌号不一致  "));
			tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 4, 0*8, "    禁止加油    ", strlen( "    禁止加油    "));
			if(id==dsp->interfaceID)	tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR_NO);
			else										tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR);
			break;
		case DSP_ETC_OILCR://szb_fj_20171120,ETC油品不一致确认
			memset(dsp_buffer,0,sizeof(dsp_buffer));
			memcpy(dsp_buffer,"此枪加注",strlen("此枪加注"));
			memcpy(dsp_buffer+8,buffer+10,10);
			tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 0, 0*8, dsp_buffer, 16);
				
			memset(dsp_buffer,0,sizeof(dsp_buffer));
			memcpy(dsp_buffer,"您上次加注",strlen("您上次加注"));
			memcpy(dsp_buffer+10,buffer,10);
			if(strlen((const char *)dsp_buffer)<=16)
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 2, 0*8, dsp_buffer, strlen((const char *)dsp_buffer));
			else if((strlen((const char *)dsp_buffer)>16) && (strlen((const char *)dsp_buffer)%2)==0)
			{
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,2, 0*8, dsp_buffer, 16);
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 4, 0*8, dsp_buffer+16, strlen((const char *)dsp_buffer)-16);
			}
			else if((strlen((const char *)dsp_buffer)>16) && ((strlen((const char *)dsp_buffer)%2)!=0))
			{
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,2, 0*8, dsp_buffer, 15);
				tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO, 4, 0*8, dsp_buffer+15, strlen((const char *)dsp_buffer)-15);
			}	
			tdDspContent(dsp->DEVKeyx, KB_FONT16, KB_CONTRAY_NO,6, 0*8,  "   请确认油品   ", 16);
			if(id==dsp->interfaceID)	tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR_NO);
			else										tdDsp(dsp->DEVKeyx, dsp->Contrast, KB_DSPCLR);
			break;
...
{
	DspParamStructType *dsp=NULL;
	dsp=&DspParamA;	//A面显示任务
	dsp->Contrast=32;//显示对比度

    //键盘点阵屏显示相关参数
	dsp->DEV=DSP_DEV_KEYBOARD;	//键盘点阵显示屏
	dsp->DEVKeyx=DEV_DSP_KEYA;	//点阵屏设备号
	...
	dsp=&DspParamB;     //B面显示任务
	dsp->Contrast=32;	//显示对比度

    //键盘点阵屏显示相关参数
	dsp->DEV=DSP_DEV_KEYBOARD;//键盘点阵显示屏	
	dsp->DEVKeyx=DEV_DSP_KEYB;//点阵屏设备号
