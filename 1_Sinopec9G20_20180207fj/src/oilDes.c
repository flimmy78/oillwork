...
#include "../inc/oilDes.h" //fj:20170905
...
void des_en3(unsigned char *key, const unsigned char *mingwen,unsigned int m_len,unsigned char *miwen)
{
  unsigned int i,iint_ks=0;
  unsigned char buf1[17],buf2[17],j;
  des2key(key,EN0);
  iint_ks=m_len/8;
  for(i=0;i<iint_ks;i++)
  {
	for(j=0;j<8;j++)  {buf1[j]=*(mingwen+i*8+j);}
    Ddes(buf1,buf2);
	for(j=0;j<8;j++)  {*(miwen+i*8+j)=buf2[j];}
  }
...
extern "C" void WINAPI des_en1(UCHAR *key,UCHAR *mingwen,int m_len,UCHAR *miwen)
{
  int i,iint_ks=0;
  deskey(key,EN0);
  iint_ks=m_len/8;
...
extern "C" void WINAPI des_de1(UCHAR *key,UCHAR *miwen,int m_len,UCHAR *mingwen)
{
  int i,iint_ks=0;
  deskey(key,DE1);
  iint_ks=m_len/8;
...
//3DES加密  key:16字节 m_len：8的倍数
extern "C" void WINAPI des_en3(UCHAR *key,UCHAR *mingwen,int m_len,UCHAR *miwen)
{
  int i,iint_ks=0;uchar buf1[17],buf2[17];
  des2key(key,EN0);
  iint_ks=m_len/8;
  for(i=0;i<iint_ks;i++)
  {
...
extern "C" void WINAPI des_de3(UCHAR *key,UCHAR *miwen,int m_len,UCHAR *mingwen)
{
  int i,iint_ks=0;uchar buf1[17],buf2[17];
  des2key(key,DE1);
  iint_ks=m_len/8;
